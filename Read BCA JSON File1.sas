...
Data Work.&JSON Work.X2;
	...
	Length Hierarchy1 $ 1000;

	If Reverse(Scan(Reverse(Trim(Left(Hierarchy))),1,'-')) = 'required' then Flag = 'Mandatory';
	Else Flag = 'Optional';
	...
	Do i = 1 to P;
		...
		If i = P then
		Do;
			Hierarchy1 = Compress(Trim(Left(Hierarchy))||'-'||Trim(Left(Col{i})));
*--- Remove properties- from the Data_Element variable ---;
			Hierarchy1 = Trim(Left(Tranwrd(Hierarchy1,'properties-','')));
...
/*	Data_Element = Compress(Tranwrd(Tranwrd(Tranwrd(Tranwrd(Tranwrd(Tranwrd(Hierarchy,'data-',''),'properties-',''),'-enum',''),'-items',''),'-required',''),'items-',''));*/
	...
	Hierarchy = Compress(Tranwrd(Tranwrd(Tranwrd(Tranwrd(Tranwrd(Hierarchy1,'data-',''),'properties-',''),'-enum',''),'-items',''),'-required',''));

Run;


Data Work.&JSON Work.X3;
	Set Work.&JSON;

	Length New_Data_Element1 New_Data_Element2 New_Data_Element3 New_Data_Element4 New_Data_Element5 New_Data_Element6 $ 1000;
...
Data Work.&JSON Work.X5;
	...
	Length Columns $ 30 Value New_Data_Element $ 1000;
